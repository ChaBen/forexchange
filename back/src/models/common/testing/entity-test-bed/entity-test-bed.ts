...
import { gt, gtStrictly, lte } from '../../../../common/big-number-util';
 ...
 * ```
 */
export type EntityAssertion<T extends BaseEntity> = Partial<
  Record<keyof T, any>
>;
...
export class EntityTestBed {
  ...
  static async createEntity<T>(
    entity: ObjectType<T> | string,
    value: CreateEntityValue<T>,
  ): Promise<T>;
  static async createEntity<T>(
    entity: ObjectType<T> | string,
    value: CreateEntityValue<T>[],
   ...
   * ```
   */
  static assertEntity<T extends BaseEntity>(
    entity: T,
    expectation: EntityAssertion<T>,
  ): void {
   ...
   * ```
   */
  static async assertDatabase(
    previous: DatabaseSnapshot,
    difference: DatabaseAssertion,
  ): Promise<void> {
  ...
  async _cleanup(): Promise<void> {
    ...
    try {
      ...
      console.error(
        '[EntityTestBed] Error occurred when closing core database connection.',
        e,
      );
      throw e;
    }
  ...
  async _clear(): Promise<void> {
    ...
    try {
      await Promise.all(
        allEntityTypes.map(async entity =>
          this._getManager()
  ...
  async _createEntity<T>(
    ...
    for (const v of values) {
      ...
      for (const member of Object.entries(v)) {
        ...
        (<any>activated)[member[0]] =
          typeof member[1] === 'function' ? member[1]() : member[1];
      }
  ...
  async _getDatabaseSnapshot(): Promise<DatabaseSnapshot> {
    const resultAsArray: BaseEntity[][] = await Promise.all(
      allEntityTypes.map(async e => this._getManager().find(e)),
    );

    const resultAsObject: DatabaseSnapshot = {};
    resultAsArray.forEach(
      (entities, i) => (resultAsObject[allEntityTypes[i].name] = entities),
    );
  ...
  async _restoreFromSnapshot(snapshot: DatabaseSnapshot): Promise<void> {
    ...
    const entries: [EntityClass, BaseEntity[]][] = Object.entries<BaseEntity[]>(
      snapshot,
    )
      .filter(([_, records]) => 0 < records.length)
      .map(
        ([name, records]) =>
          [getEntityClassByName(name), records] as [EntityClass, BaseEntity[]],
      );
    const createEntities = entries
      .filter(([className]) => allEntityTypes.includes(className))
      .map(async ([className, records]) =>
        this._createEntity(className, records),
      );
 ...
 * @private
 */
function assertEntity<T extends BaseEntity>(
  entity: T,
  assertion: EntityAssertion<T>,
  parentKey: string = '',
): void {
  for (const [key, assertionValue] of Object.entries(assertion)) {
    ...
    try {
      ...
      throw new AssertionError(
        `"${parentKey +
          key}" is incompatible. expected: ${toLog}, actual: ${actualValue}`,
      );
    }
  }
}
...
async function assertDatabase(
  ...
  for (const data of Object.entries<TableAssertion>(databaseAssertion)) {
    ...
    if (!Array.isArray(previousEntities)) {
      throw new Error(
        `provided snapshot does not have records for expectation, ${tableName}`,
      );
    }
    const currentEntities = await EntityTestBed.getManager().find<BaseEntity>(
      tableName,
    );
    ...
    if (tableAssertion.created) {
      const newlyCreatedEntities = currentEntities.filter(record =>
        gtStrictly(record.createdAt, latestUpdatedTime),
      );
      const createAssertions = tableAssertion.created.assertion || [];

      if (
        typeof tableAssertion.created.count === 'number' ||
      ...
      for (const assertion of createAssertions) {
        const errorMessages = getFailedAssertionMessages(
          newlyCreatedEntities,
          assertion,
        );
        const allAssertionFailed =
          errorMessages.length === newlyCreatedEntities.length;
    ...
    if (tableAssertion.updated) {
      const newlyUpdatedEntities = currentEntities.filter(
        e =>
          gt(e.updatedAt, latestUpdatedTime) &&
          lte(e.createdAt, latestUpdatedTime),
      );
      const updateAssertions = tableAssertion.updated.assertion || [];

      if (
        typeof tableAssertion.updated.count === 'number' ||
      ...
      for (const [findOption, assertion] of updateAssertions) {
        ...
        if (target === undefined) {
          throw new Error(
            `no record found for previous. \n  findOption: ${JSON.stringify(
              findOption,
            )}`,
          );
        }
    ...
    if (tableAssertion.deleted) {
      const newlyDeletedEntities = previousEntities.filter(
        e1 => !currentEntities.some(e2 => e1.id === e2.id),
      );
      const deleteAssertions = tableAssertion.deleted.assertion || [];

      if (
        typeof tableAssertion.deleted.count === 'number' ||
      ...
      for (const assertion of deleteAssertions) {
        ...
        const allAssertionFailed =
          errorMessages.length === newlyDeletedEntities.length;
        if (allAssertionFailed) {
          throw new Error(
 ...
 * @param assertion
 */
function getFailedAssertionMessages<T extends BaseEntity>(
  entities: T[],
  assertion: EntityAssertion<T>,
): string[] {
  const errors: string[] = [];
